...
If you get a particularly large spike of updates to your indexed models, you might be taxing your Solr server with lots of single document updates. This could lead to downstream performance issues in your application. By queueing the Solr updates, they will be sent to the server in larger batches providing better performance. Furthermore, if Solr gets backed up updating it's index, the slow down will be isolated to the background job processing the queue.


== Usage

To use asynchronous indexing, you'll need to set up two different parts.

=== Session

Your application should be initialized with a SessionProxy session. This session is set up to immediately process queries but send updates to a queue for processing later.

This will set up a session proxy using the default Solr session:

  Sunspot.session = Sunspot::IndexQueue::SessionProxy.new

If you have custom configuration settings for your session or need to wrap it in additional proxies, you can pass the session to wrap into the constructure. For example, if you have a master/slave setup running on specific ports:

  master_session = Sunspot::Session.new{|config| config.url = 'http://master.solr.example.com/solr'}
  slave_session = Sunspot::Session.new{|config| config.url = 'http://slave01.solr.example.com/solr'}
...
When you have updates coming from multiple sources, it is often good to set a priority on when they should get processed so that when there is a backlog, it becomes less noticeable. For example, in an application that has models updated both by human interaction and by an automated feed, you probably want the human updated items to be indexed first. That way when their is a backlog, your human customers don't notice it and don't complain. You can control the priority of updates inside a block with the IndexQueue.set_priority method.

=== Recoverability

When an entry in the queue cannot be sent to Solr, it will be automatically rescheduled to be tried again later. The amount of time is controlled by the +retry_interval+ setting on IndexQueue (default to 1 minute). Every time it is tried and fails, the interval will be increased yet again (i.e. first try wait 1 minute, second try wait 2 minutes, third try wait 3 minutes, etc.).

Error messages and stack traces are stored with the queue entries so the can be easily debugged.


== Caveats

By updating your Solr index asynchronously, it will be eventually consistent, but could be temporarily out of sync with your data store. The biggest issue to watch out for is that your index could return results for objects that have already been deleted. Make sure that the Sunspot::Adapters::DataAccessor you are using accounts for nils (i.e. it shouldn't just blindly load objects and assume they will be there). Otherwise, you could end up with nil pointer exceptions in your application.
